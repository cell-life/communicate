...
public class PostCommandTest extends BaseHttpTest {

	private UrlPostHandler handler;
	
	@Override
	protected void registerHandlers(LocalTestServer localServer) {
		handler = new UrlPostHandler("POST", Status.CREATED.getStatusCode());
	...
	public void testPostCommand_basic(){
		...
		try {
			new PostCommand(getServerUrl(), "simpleurl", "1234abcd").execute(String.class);
	...
	public void testGetCommand_basic1() throws PropertyException, JAXBException{
		...
		try {
			new PostCommand(getServerUrl(), "simpleurl", dto).execute(String.class);
		} catch (RestCommandException e) {
			Assert.fail(e.getMessage());
		}
	}
