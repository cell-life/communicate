...
public enum SMPPErrorCodes {
	...
	ESME_RDELIVERYFAILURE(0x000000FE,"Delivery Failure (used fordata_sm_resp)"),
	ESME_RUNKNOWNERR(0x000000FF,"Unknown Error");
	
	private final int code;
	private final String description;

	private SMPPErrorCodes(int code, String description) {
		this.code = code;
	...
	public static SMPPErrorCodes getFromCode(int code){
		for (SMPPErrorCodes err : values()) {
			if (err.getCode() == code){
				return err;
			}
		}
