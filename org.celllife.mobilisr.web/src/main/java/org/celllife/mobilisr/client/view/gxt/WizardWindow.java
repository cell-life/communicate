...
public class WizardWindow extends Window {
		...
		if (button == nextBtn) {
			...
			AsyncCallback<Void> validationCallback = new AsyncCallback<Void>() {
				...
				public void onFailure(Throwable caught) {
					if (caught != null)
						MessageBoxWithIds.alert("Validation Error", caught.getMessage(), null);
				}
				...
				public void onSuccess(Void result) {										
					if (currentStep + 1 == cards.size()) {		
						...
						toolBar.disable();
						cards.get(currentStep).fireEvent(MobilisrEvents.WizardStep, new WizardStepEvent(MobilisrEvents.WizardStep));
						cards.get(currentStep).fireEvent(MobilisrEvents.WizardFinish);
						fireEvent(MobilisrEvents.WizardFinish, new BaseEvent(MobilisrEvents.WizardFinish));
						if (hideOnFinish)
							hide();
					} else {
						cards.get(currentStep).fireEvent(MobilisrEvents.WizardStep, new WizardStepEvent(MobilisrEvents.WizardStep));
	...
	private void updateWizard(int previousStep) {		
		WizardStepEvent be = new WizardStepEvent(MobilisrEvents.WizardStep);
		...
		if (! fireEvent(MobilisrEvents.WizardStep, be)){
			return;		
		}
